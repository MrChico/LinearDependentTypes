...
\end{lem}
\begin{thm}[Soundness]
A split comprehension category $p : \mathcal{T} \to \mathcal{C}$ together with a split lax monoidal fibration $q : \mathcal{L} \to \mathcal{C}$ is a model for the linear dependent type theory consisting of the structural rules presented in Figure \ref{struc}.
\begin{proof}
...
\begin{defn}A model of LDTT \textbf{supports $\Sigma$-types} if it satisfies the following:
  ...
  \item such that for all pullbacks (as in \ref{pb-diag}), the natural transformation: $\Sigma_Eq^* \to f^*\Sigma_{E'}$ is a natural isomorphism, and
