...
\newpage
\section{TODO}
Prove that $M \vdash L$ is an adjunction using Peters supplied rule. Can one prove that it is monoidal?
Try to find examples of linear types from programming - modify in place etc
Consider introducing a cartesian ``linear hom-set'' type to model the V-enriched categories
...
\]
are not admissable.\\
In other words, assumptions cannot be freely assumed or dismissed; they must be used exactly once in the conclusion. In linear type theory, we can think of a sequent $x_1 : A_1, x_2 : A_2, \dots, x_n : A_n \vdash t : A$ as modelling a function or process in which the assumptions $x_1, x_2, \dots x_n$ are \textit{resources used} to produce $t$. As a first, toy example, we consider the chemical process of burning hydrogen:
\begin{ex}\label{burn}
...
Since we allow linear types to dependend on terms of cartesian types, we can form new versions of the $\Pi$- and $\Sigma$-types. This can be done in two ways, where the resulting type is either cartesian or linear.
The linear variants of $\Pi$- and $\Sigma$-types will be denoted $\sqcap$ and $\exists$, respectively.\\
\fbox{
\begin{minipage}{.5\linewidth}
\[
...
\]\\
\[
\inference{\Gamma \vdash s : A \quad \Gamma; \Xi \vdash b : B[s/x]}{\Gamma; \Xi \vdash (s, b) : \exists_{x : A}B}[$\exists$-I]
\]\\
\[
...
\[
  ...
  \end{split}
\]
If we let the resulting type be cartesian, we get the following type formers:
TODO:\\
...
\end{defn}
The semantic counterpart for the mixed types $\sqcap$ and $\exists$ is similar to that of $\Pi$ and $\Sigma$, but instead of $\mathcal{T}$, we ask for adjoints of the induced functors between fibers of $\mathcal{L}$.
\begin{defn}
...
It supports $\exists$-types if every $\pi_A^*$ has a monoidal left adjoint.
\end{defn}
\begin{defn}[Id-types]
...
First, we introduce a new type with which we can reason (non-linearly) about the hom-sets of $\mathcal{V}$:\\
\fbox{
\begin{minipage}{.5\linewidth}
  \[
...
In the presence of linear function types and the $M$-functor, this type can be constructed as $(A \multimap B)_M$, but adding it as a primitive allows us to talk about the hom-sets of $\mathcal{V}$ even when it is not closed.\\
We define the type \textbf{Enr-Cat} to consist of the following:
\begin{itemize}
\item A (cartesian) type $A$,
\item for any $x, y : A$, a linear type $B_{x, y}$,
\item a linear dependent function $M_{x, y} : \sqcap_{x, y, z} B_{x, z} \otimes B_{x,y} \multimap B_{x,z}$,
\item a linear dependent function $j_x : \sqcap_{x : A} B_{x, x}$ and 
\item proofs of the equalities $[\lambda f. f] = _{[B_{x, y}, B_{x, y}]} [\lambda f. M(f \otimes j_x)]$ and $[\lambda f. f] = _{[B_{x, y}, B_{x, y}]} [\lambda f. M(j_y \otimes f)]$
\end{itemize}
...
\[
  \begin{split}
    \cdot \vdash \Sigma_{A : U} \Pi_{M_{x, y, z} : \sqcap_{x, y, z : A} B_{y, z} \otimes B_{x, y} \multimap B_{x,z}} \Pi_{j_x : \sqcap_{x: A}B} \\
  \cdot &\vdash A \text{ type}\\
  x, y : A &\vdash B_{x,y} \linear\\
\cdot &\vdash \sqcap_{x, y : A}B_{x, y} \text{ linear}\\
x : A ; \cdot &\vdash 1_x : B_{x, x}\\
\cdot; \cdot &\vdash 1_{-} : \sqcap_{x : A}B_{x, x}\\
\cdot; \cdot &\vdash \_\circ\_ : \sqcap_{x, y, z}B_{y,z} \otimes B_{x, y} \multimap B_{x, z}\\
x, y : A, z : B_{x,yM}; \cdot &\vdash \sigma(z) \circ 1_x : B_{x, y}\\
x, y : A, z : B_{x,yM} &\vdash p : (\sigma(z) \circ 1_x)_M =_{B_{x,y}M} \sigma(z)_M\\
x, y : A, z : B_{x,yM} &\vdash q : (1_y \circ \sigma(z))_M =_{B_{x,y}M} \sigma(z)_Mz\\
x, y, z, w : A, f : {B_{x, y}}_M, g: {B_{y, z}}_M, h : B_{{z, w}M} &\vdash r : (\sigma(h) \circ (\sigma(g) \circ \sigma(f)))_M =_{B{_y,zM}} ((\sigma(h) \circ \sigma(g)) \circ \sigma(f))_M
\end{split}
\]
