...
\[
  \inference{
    ...
    \Gamma \vdash a : A_L \\}
  {
    \Gamma; \Xi, \Xi' \vdash t[a/y] \equiv t[a/y] : B\\
  }[L-U]
  \]\\
\[
...
\begin{thm}
  $M$ is a functor. I.e. for any linear types, $\Gamma \vdash A \linear$ and $\Gamma \vdash B \linear$, there is a term $fmap : M(A \multimap B) \to MA \to MB$ such that $fmap M(id_A) \equiv id_{MA}$.
  \begin{proof}
  Define:
  \[
    \Gamma, f : M(A \multimap B), a : A_M \vdash (\sigma(f)\sigma(a))_M : B_M
  \]
  and see that for $f = M(id_A)$, we have $\sigma(f)(\sigma(a)) \equiv id_A\sigma(a)_M \equiv a : A_M$.
\end{proof}
\end{thm}
\begin{thm}
  $L$ is a functor. I.e. for any cartesian types, $\Gamma \vdash A \type$ and $\Gamma \vdash B \type$, there is a term $fmap : L(A \to B) \multimap LA \multimap LB$ such that $fmap L(id_A) \equiv id_{LA}$.
  \begin{proof}
  Define:
  \[
    \begin{split}
    \Gamma, f : L(A \to B), y : A; \cdot &\vdash f(y)_L : B_L\\
    \Gamma, f : L(A \to B); a : A_L &\vdash \text{let $y$ be $a$ in $f(y)_L$} : B_L\\
    \Gamma; \hat f : L(A \to B), a : A_L &\vdash \text{let $\hat f$ be $f$ in (let $y$ be $a$ in $f(y)_L$)}\\
  \end{split}
  \]
\end{proof}
\end{thm}
and now for $\hat f = L(id_A)$, we have $\text{let $\hat f$ be $(id_A)_L$ in (let $y$ be $a$ in $f(y)_L$)} \equiv \text{let $y$ be $a$ in $\hat f(y)_L$}$\\
...
\end{proof}
\end{thm}
It is hard to reason about equality of linear terms since the ordinary $Id$-type is a dependent type, and in the theory we do not allow for type dependency on linear terms. In the presence of the $M$ operator, however, we can form the type $a_M = b_M$ for two terms of the same linear type, which, if $M$ is faithful, serves as a surrogate for linear equality. However, as will be demonstrated by theorem \ref{M-faith}, faithfulness of $M$ is independent of the rest of the theory. This means that one can consistently assume the following theorem (or its negation):
\[
  \inference{
    \Gamma \vdash A \linear\\
    \Gamma; \cdot \vdash a : A\\
    \Gamma; \cdot \vdash b : A\\
    \Gamma \vdash a_M \equiv b_M : A_M}
  {\Gamma \vdash a \equiv b : A}[M-faith]
\]
\begin{comment}
...
But with linear universes, we can formulate a type level equality of linear types. This means that we can also formulate a univalence axiom \cite{voed} for linear types. First, we define the identity function:
\[
  id : \sqcup_{A : \mathcal{L}}
\]


\[
  equiv(A, B) = \Pi_{f : A \multimap B}
\]
...
\end{defn}
  It will sometimes be useful to also assume that the comprehension category has a \textit{unit}, as this give us an isomorphism $\mathcal{C}_\Gamma(\pi(A), \pi(B)) \cong \mathcal{T}_{\Gamma.A}(1, \pi^*_A*(B))$ \cite{jacobs}.
\begin{defn}
  ...
  \end{defn}
  Most of our models will naturally be equipped with such a structure, and this yields the nice property that $\mathcal{C}_{/\Gamma}(\Gamma.A, \Gamma.B) \cong \mathcal{T}_{\Gamma.A}(1, \pi_A^*(B))$, allowing us to shift in perspective to think of terms depending on $\Gamma$ and $A$ as either living in the slice category $\mathcal{C}_{/\Gamma}$ or in the fiber $\mathcal{T}_{\Gamma.A}$. Importantly, since $\mathbf{1}$ is full and faithful, the unit of the adjunctionis an isomorphism, meaning that for every $A \in \mathcal{T}_\Gamma$, we have $eta_A : A_{\Gamma} \cong 1_{\Gamma.A}$. But as these objects live in different slices, the following result is more convenient
  \begin{thm}
    For any $A$ over $\Gamma$, we have that $1_{\Gamma.A} \cong \pi_A^*(A)$.
    \begin{proof}
The cartesian property of the lift of $\pi_A$ gives rise to the unique arrow $v$ in the following diagram:
      \[
        \ti
        1_{\Gamma.A} \ar[rd, "\eta_A^{-1}"] \ar[d, "\exists!v"] & \\
        \pi^*_A(A) \ar[r, "\pi_A^*"] & A\\
        \kz
        \]
    \end{proof}
      Since $1_{\Gamma.A}$ is a terminal object in the same slice as $\pi_A^*(A)$, there also exists a unique arrow $v$ in the other direction. Since $uvu = u$ and $vuv = v$ they form an isomorphism.
  \end{thm}
...
\begin{defn}\label{semanticML}
  A model of LDTT with unit and simple products \textbf{supports the operators $M$ and $L$} if there exists monoidal functors $M : \mathcal{L} \leftrightarrow \mathcal{T} : L$ which are cartesian with respect to the fibrations $p : \mathcal{T} \to \mathcal{C}$ and $q : \mathcal{L} \to \mathcal{C}$, such that $L \dashv M$ is a fibred, monoidal adjunction such that:
  \begin{enumerate}
  \item There is an isomorphism of hom-sets $\mathcal{L}_{\Gamma.A}(\pi_A^*(\Xi'), \pi_A^*(B)) \cong \mathcal{L}_\Gamma(LA \otimes \Xi', B)$.
    \end{enumerate}
\end{defn}
...
The final condition of the definition is what yields the elimination and computation rules L-U, and while it might appear somewhat unnatural semantically, it turns out to hold in a large class of models, satisfying that the reindexing functors $\pi_A^*$ of the monoidal fibration are full and faithful.
\begin{thm}For all $A \in \mathcal{T}_\Gamma$, if $\pi_A^* : \mathcal{L}_\Gamma \to \mathcal{L}_{\Gamma.A}$ is full and faithful, then any strong monoidal fiber adjunction $L \vdash M$ satisfies $\mathcal{L}_{\Gamma.A}(\pi^*(\Xi'), \pi^*(B)) \cong \mathcal{L}_\Gamma(LA \otimes \Xi', B)$.
  \begin{proof}
    Since $1_{\Gamma.A} \cong \pi^*_A A$ in $\mathcal{T}_{\Gamma.A}$, we have $I \cong L(1) \cong L_{\Gamma.A}(\pi_A^* A) \cong \pi_A^*(L_{\Gamma}A)$ in $\mathcal{L}_{\Gamma.A}$. By this there is an isomorphism of homsets:
    \[
      \begin{split}
        \mathcal{L}_{\Gamma.A}(\pi^*_A\Xi', \pi^*_AB) \cong \\
        \mathcal{L}_{\Gamma.A}(I \otimes \pi^*_A\Xi', \pi^*_AB) \cong \\
        \mathcal{L}_{\Gamma.A}(\pi^*LA \otimes \pi^*_A\Xi', \pi^*_AB) \cong \\
        \mathcal{L}_{\Gamma}(LA \otimes \Xi', B)
        \end{split}
    \]
  \end{proof}
\end{thm}
The requirement that the reindexing functors are full and faithful is a reasonable requirement from a syntactic perspective, as it says that no terms are lost nor gained from weakening the cartesian part of the context in a linear judgement. It comes as no surprise then, that another sitatution when we can support $M$ and $L$ ``for free'' is in the presence of semantic $[-,-]$ type formers.
\begin{thm}In a model of LDTT that supports $[-,-]$ type formers, then any strong fibred monoidal adjunction $L \dashv M$ satisfies $\mathcal{L}_{\Gamma.A}(\pi^*_A(\Xi'), \pi^*(B)) \cong \mathcal{L}_{\Gamma}(LA \otimes \Xi', B)$.
  \begin{proof}
    A model supporting internal homs must have reindexing functions which preserve these. That is, we havean isomorphism $\pi_A^*[A, B] \cong [\pi_A^*A, \pi^*B]$. We get a chain of isomorphisms:
\[
      \begin{split}
...
\begin{thm}\label{M-faith}Faithfulness of $M$ is independent.
  \begin{proof}
    We have already seen a model for when the functor $M$ is faithful, for example in the families model for any concrete monoidal category $\mathcal{V}$. A model where $M$ is not faithful is given by taking the fibration over families of groupoids $q : [-, \mathbf{Gpd}] \to \mathbf{Gpd}$ as both the cartesian and split monoidal fibration, and defining $L = \delta \pi_0$ and $M = \delta U$, where $U$ is the underlying set functor. We have that $L \vdash M$ is a monoidal fiber adjunction, but $M$ is not faithful.
  \end{proof}
\end{thm}
