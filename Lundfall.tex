...
For a fibration $p : Fam(\mathcal{C}) \to \mathbf{Sets}$ and a function $u : S \to S'$ in the base, $u^* : Fam(\mathcal{C})_{S'} \to Fam(\mathcal{C})_S$ has a left (right) adjoints if and only if $\mathcal{C}$ has small coproducts (products).
  ...
  \end{proof}
\end{thm}
Since $\mathbf{Sets}$ is complete and co-complete, we have left and right adjoints to all reindexing functors. These will serve as the functors $\Pi$ and $\Sigma$ supporting the corresponding types. It remains to check that these satisfy the Beck-Chevalley condition, and that there exists an isomorphism $\Gamma.\Sigma_AB \cong \Gamma.A.B$.\\
\begin{thm}
  The left and right adjoints to $u^*$ described in \ref{adjointproducts} form support for the $\Pi$ and $\Sigma$ type formers in the families model.
  \begin{proof}
    Let the right adjoint functor be called $\Pi$. First we show that $\Pi$ satisfies the Beck-Chevalley condition:\\
  Given a pullback square in $\mathbf{Set}$ of the following form:
  \[
    \ti
        \Gamma.E \ar[d, "\pi_E"] \ar[r, "q_{E, E'}"] & \Delta.E' \ar[d, "\pi_{E'}"] \\
    \Gamma \ar[r, "f"] & \Delta
    \kz
  \]
  the natural transformation $\alpha :f^*\Pi_{E'} \to \Pi_Eq^*_{E,E'}$ arises from the image of the counit $\epsilon : \pi_{E'}\Pi_{E'} \to 1_{\mathcal{T}_{\Delta.E'}}$ under the functor $q^*_{E,E'}$:
  \[
      q^*_{E,E'}{\epsilon} : q^*_{E,E'}\pi_{E'}^*\Pi_{E'} \to q^*_{E, E'}\\
  \]
Transported via the isomorphism:
\[
  \begin{split}
&    \mathcal{T}_{\Gamma.E}(q^*_{E,E'}\pi_{E'}^*\Pi_{E'}(C), q^*_{E, E'}(C)) =\\
&    \mathcal{T}_{\Gamma.E}(\pi_{E}^*f^*\Pi_{E'}(C), q^*_{E, E'}(C)) \cong\\
&    \mathcal{T}_{\Gamma.E}(f^*\Pi_{E'}(C), \Pi_Eq^*_{E, E'}(C)) \\
    \end{split}
  \]
  so $\alpha$ is an isomorphism iff $\epsilon$ is, which holds iff $\Pi$ is a full and faithful functor, so we need to display an isomorphisms of homsets:
  \[
    \begin{split}
      \mathcal{T}_{\Gamma.E}((\Gamma.E, f), (\Gamma.E, g)) \cong \\
      \mathcal{T}_{\Gamma}(\Pi_E(\Gamma, f), \Pi_E(\Gamma, g))\ = \\
       \mathcal{T}_{\Gamma}\left ((\Gamma, \lambda \gamma. \prod_{(\gamma, e)}f(\gamma, e)), (\Gamma, \lambda \gamma. \prod_{(\gamma, e)}g(\gamma, e)) \right)
    \end{split}
  \]
  Now, a map $\beta : (\Gamma.E, f) \to (\Gamma.E, g)$ consists of morphisms $\beta_{(\gamma, e)} : f(\gamma, e) \to g(\gamma, e)$, so precomposing these with the projections $\pi_{f(\gamma,e)} : \prod_{(\gamma, e)}f(\gamma,e) \to f(\gamma, e)$ yields by the universal property of the product a unique set of maps $\bar \beta_\gamma : \prod_{(\gamma,e)}f(\gamma,e) \to \prod_{(\gamma,e)}g(\gamma,e)$ such that $\bar \beta_\gamma \pi_{g(\gamma,e)} = \beta_{(\gamma,e)}\pi_{f(\gamma,e)}$.
\end{proof}
\end{thm}


that and that it supports $\sqcap$ (or $\sqsubset$) if and only if $\mathcal{V}$ has small products (or coproducts).\\
...
Note that this procedure of enlarging the base category can also be used to account for cases where the category $\mathcal{V}$ is large, given the existence of enough inaccessible cardinals.
...
Armed with these results, we can prove an important first result of the higher dimensional model, answering 
  ...
  \begin{thm}
    ...
    \end{proof}
  \end{thm}
  \subsubsection{Univalent linear type theory}
  Equipped with the new linear univalence axiom, we are able to prove things that linear logicians have long taken to be true.
  \begin{thm}
    Assuming L-ua, for all linear types $A$, $B$ and $C$ in $\Gamma$, the following types are inhabited:
    \begin{enumerate}
...
and $h^{-1} : (A \otimes B \multimap C) \multimap (A \multimap B \multimap C)$:
  ...
  should be able to prove a range of fruitful results:
  \[
    \begin{split}
      A \otimes B \multimap C &= A \multimap (B \multimap C)\\
      (\sqcap_{x : A}B)_M &= \Pi_{x : A}B_M\\
      (\sqcup_{x : A}B)_M &= \Sigma_{x : A}B_M
    \end{split}
  \]
