...
\newpage
\section{TODO}
More careful about size issues wrt universes and the adjunction $M$ and $L$.
Linear univalence
...
\begin{thm}\label{extensionalid}
  ...
  \begin{proof}
    ...
    Identities:
    \[
     \pi_{A\{\pi_A\}} \pi_{Id_A}r_A = 1_{\Gamma.A}
    \]
    yielding
    \[
      r_A^*\pi_{Id_A}^*\pi_{A\{\pi_A\}}^* = 1_{\mathcal{L}_{\Gamma.A}}
    \]
    \[
      \pi_{Id_A}r_A = v_A
    \]
    yielding
    \[
      r_A^*\pi_{Id_A}^* = v_A^* : \mathcal{L}_{\Gamma.A.A\{\pi_A\}} \to \mathcal{L}_{\Gamma.A}
    \]
    
    We can transport the map $c$ via the functors $\pi_{A\{\pi_A\}}^* : \mathcal{L}_{\Gamma.A} \to \mathcal{L}_{\Gamma.A.A\{\pi_A\}}$ and $\pi_{Id_A}^* : \mathcal{L}_{\Gamma.A.A\{\pi_A\}} \to \mathcal{L}_{\Gamma.A.A\{\pi_A\}.Id_A}$ to get the map:
\[
\pi_{Id_A}^*\pi_{A\{\pi_A\}}^*(c) : \pi_{Id_A}^*\pi_{A\{\pi_A\}}^*r_A^*\Xi \to \pi_{Id_A}^*\pi_{A\{\pi_A\}}^*r_A^*C
\]
Then, transporting this map back via the functors $P^+ : \mathcal{L}_{\Gamma.A.A\{\pi_A\}.Id_A} \to \mathcal{L}_{\Gamma.A.A\{\pi_A\}}$, $N^+ : \mathcal{L}_{\Gamma.A.A\{\pi_A\}} \to \mathcal{L}_{\Gamma.A}$ and $M : \mathcal{L}_{\Gamma.A} \to \mathcal{L}_{\Gamma}$ gives the map:
\[
M^*{N^+}^*{P^+}^*\pi_{Id_A}^*\pi_{A\{\pi_A\}}^*(c) : M^*{N^+}^*{P^+}^*\pi_{Id_A}^*\pi_{A\{\pi_A\}}^*r_A^*\Xi \to M^*{N^+}^*{P^+}^*\pi_{Id_A}^*\pi_{A\{\pi_A\}}^*r_A^*C
\]
which, since $P^+$ and $N^+$ are sections to the corresponding projections, reduces to:
\[
M^*c : M^*{N^+}^*{P^+}^*\pi_{Id_A}^*\pi_{A\{\pi_A\}}^*r_A^*\Xi \to M^*{N^+}^*{P^+}^*\pi_{Id_A}^*\pi_{A\{\pi_A\}}^*r_A^*C
\]
\end{comment}
...
\begin{defn}
  We say that a model of linear dependent type theory \textbf{supports $M$ and $L$ for small types}, if it supports $M$ and $L$ for a submodel of small types.
\end{defn}
...
\subsubsection{Universes in the families model}
...
By this construction, we recover the ``small'' families model as submodel of small types. Furthermore, by restricting the type formers $M$, $L$ $\sqsubset$ and $\sqcap$ to only be defined for small types, this linear universe is all-encompassing.
...
\]
  ...
  and in the other direction, given an identity $p : A =_L B$, turning it to an equivalence and then back into an identity should return $p$:
    \[
    \inference{
      \Gamma \vdash ua(f)_{[g, h, p, q]}(R^{Id}_{[x, y, p]}(\lambda u.u, A, B, p))_{[-,-,-,-]} : A =_L B}
    {\Gamma \vdash ua(f)_{[g, h, p, q]}(R^{Id}_{[x, y, p]}(\lambda u.u, A, B, p))_{[-,-,-,-]} \equiv p : A =_L B}
    [L-ua-C$_2$]
  \]\\
  ...
  \begin{lemm}\label{compiscomp}
    The syntactic composition operation for linear types, $\_\circ \_$, defined in \ref{syntcomp}, corresponds to the semantic operation of composition. More explicitly, given two terms $\Gamma; \cdot \vdash f : A \multimap B$, and $\Gamma; \cdot \vdash g : B \multimap C$, their interpretations $[[f]] : I \to [A, B]$ and $[[g]] : I \to [B, C]$ give rise to unique morphisms $\hat [[f]] : A \to B$ and $\hat [[g]] : B \to C$ via the tensor-hom adjunction. The interpretation of $\Gamma; \cdot g \circ f : A \multimap C$ is a morphism $[[f \circ g]] : I \to [A, C]$, giving rise to $\hat [[f \circ g]] : A \to C$. The composite $[[\hat g]] \circ [[\hat f]$ and $\hat [[f \circ g]]$ coincide. 
    \begin{proof}Naturality like crazy.
      \end{proof}
\end{lemm}
  \begin{lemm}Given two sections $A, B : \Gamma \to \Gamma.L$, and a section $p : \Gamma \to \Gamma.Id_L\{A\}\{B^+\}$, there is an natural isomorphism $IdToEquiv(p) : El(A) \cong El(B)$ in $[\Gamma, \mathcal{V}]$.
    \begin{proof}The section $p : \Gamma \to \Gamma.Id_L\{A\}\{B^+\}$ implies that...
    \end{proof}
  \end{lemm}
  ...
  \begin{thm}
    ...
    \begin{proof}
      Since $M$ factors through sets, the interpretation of any type in its image $M(C) : \Gamma \to \mathbf{Gpd}$ is a functor with values in discrete groupoids, so by lemma \ref{discgroupoid}, the existence of a section $p : \Gamma \to \Gamma.Id_{M[El(A), El(A)]}\{(g \circ f)_M\}\{(id_{El(A)})_M\}$ implies that the sections $(g \circ f)_M : 1 \to M[A, A]$ and $(id_{El(A)})_M : 1 \to M[A,A]$ coincide, which by the adjunction $L \dashv M$ implies that $[[(g \circ f)]] = id_{El(A)}$. By lemma \ref{compiscomp}, and from the fact that the term $id_{El(A)}$ corresponds to the identity morphism $1_{El(A)} : El(A) \to El(A)$ via the tensor hom adjunction, this yields an isomorphism $f : El(A) \to El(B)$ in $\diag{V}_\Gamma$, with $g = h$ as inverse. Selecting this isomorphism in $core(\mathcal{V})$ gives rise to a section $P : \Gamma \to \Gamma.Id_L\{A\}\{B\}$.
\end{proof}
\end{thm}
So the linear univalence axiom holds in the diagrams model as long as $M$ factors through sets. But this is not very satisfying from the perspective of homotopy type theory, since we simply showed this by collapsing all of our linear types to sets. Instead, we want to imagine a model where our linear types have an inherently higher dimensional structure. The first step to such a model is to let the objects of $\mathcal{V}$ to be categories, and $M$ be the functor taking a category to its core, i.e. its maximal sub-groupoid. If we want $\mathcal{V}$ to have a non-cartesian monoidal structure the candidate choices $\mathbf{Cat}$ and $\mathbf{Gpd}$ won't do. As briefly outlined in in \ref{smcatmon} and carefully described in \cite{smcat}, there is a symmtetric monoidal structure on $\mathbf{SMCat}$, the category of small symmetric monoidal categories, symmetric monoidal functors and monoidal natural transformations. \footnote{Technically, the structure on $\mathbf{SMCat}$ is not quite symmetric monoidal, as the associators, unitors and symmetry functors are only invertible up to higher homotopy. However, if one applies these homotopies whenever necessary, one does get a model of linear dependent type theory.}
\begin{defn}
...
\ti
  ...
  The linear universe $L$, we take to be the groupoid consisting of all \textit{discrete} symmetric monoidal categories, and isomorphisms between them. This is equivalently the core of $CMon$, the category of small commutative monoids (in $\mathbf{Set}$). $CMon$ is a full subcategory of $SMCat$, which allows us to let $El$ be the function sending an object of $L$ to an object of $\mathbf{SMCat}$.\\
  More explicitly, over any groupoid $\Gamma$, let $\mathcal{L}$ be the functor $L : \Gamma \to \mathbf{Gpd}$, which is constant at $core(CMon)$. A section $t : \Gamma \to \Gamma.L$ of its corresponding projection, selects for each $\gamma$, an object $a_\gamma \in core(CMon)$ and for each morphism $\alpha : \gamma \to \gamma'$, an isomorphism $a_\gamma \to a'_{\gamma'}$. This defines a functor $\hat t : \Gamma \to core(CMon)$, which we embedd into $\mathbf{SMCat}$ to get our interpretation of $El(t) : \Gamma \to \mathbf{SMCat}$.
  \begin{comment}
  Armed with these results, we can prove an important first result of the higher dimensional model, answering 
