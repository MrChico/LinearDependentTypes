...
\fbox{
  \begin{minipage}{.5\linewidth}
    \[
      \inference{\vdash \Gamma \text{ ctxt}}{\Gamma \vdash U}[$\mathcal{U}$-F]
    \]\\
    \[
      \inference{\Gamma \vdash t : U}{\Gamma \vdash El(t) \type}[El-F]
    \]\\
    \[
      ...
      \]
      \[
        \inference{\Gamma \vdash \hat \Pi_{x : El(t)}v}
        {\Gamma \vdash \hat El(\Pi_{x : El(t)}v) \equiv \Pi_{x L El(t)}El(v)}[U-$\Pi$-Ty]
      \]
